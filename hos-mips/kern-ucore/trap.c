	...
	for (i = 0; i < 8; i++) {
		if (tf->tf_cause & (1 << (CAUSEB_IP + i))) {
			...
			case 6:{
				...
				// PRINT_HEX("pic_status \r\n $", pic_status);
				//corrected by xiaohan! Accoring to priority code.
				//kprintf("COM1\n");
				// COM1 serial
				if(pic_status == INTC_COM1)
					serial_int_handler(NULL); //对应In5为1?
				// COM2 bluetooth
...
//                                break;
			default:
				PRINT_HEX("\r\n $case default cause:", tf->tf_cause);
				print_trapframe(tf);
