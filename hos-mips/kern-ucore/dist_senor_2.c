...
static bool com4_exists = 0;


static void com4_init(void)
{

    volatile unsigned char *uart = (unsigned char *)COM4;
    if (com4_exists)
        return;
    com4_exists = 1;

    outw(COM4 + COM_IER, COM_IER_RDI);
    delay();
...
static int com4_proc_data(void)
{
    int c;
    delay();
    if((inw(COM4 + COM_LSR) & COM_LSR_DATA) == 0)
        return -1;
    delay();
    c = inw(COM4 + COM_RBR) & 0xFF;
    delay();
    return c;
}

void distSensor_2_int_handler(void *opaque)
{
    turnoff_intc(INTC_COM4);
    int c = com4_proc_data();
    PRINT_HEX("\n\r ReceivedData: ", c);
    if(c == 0)
    {
        kprintf("Sensor2!\n\r");
    ...
    turnon_intc(INTC_COM4);
}

void distSensor_2_init(void)
{
    com4_init();
    if(!com4_exists)
    {
